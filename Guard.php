...
class Guard{
    ...
    public static function run(){
        if(!self::stop($_GET,self::$getFilter))                      return false;
        parse_str(file_get_contents("php://input"), $_PUT);
        if(!self::stop(array_merge($_POST,$_PUT),self::$postFilter)) return false;
        if(!self::stop($_COOKIE,self::$cookieFilter))                return false;
        if(!self::stop($_SERVER['HTTP_REFERER'],self::$postFilter))  return false;
        return true;
    }
     ...
     */
    private static function stop($data,$filter)
    {
        foreach ( $data as $k=>$v ){
            $v = self::parseParams($v);
            if (preg_match("/".$v."/is",$filter)==1){
                return false;
            }
            if (preg_match("/".$k."/is",$filter)==1){
                return false;
            }
        }
        return true;
